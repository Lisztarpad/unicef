...
with st.sidebar:
    ...
    st.subheader("💾 Export")
    export_filename = st.text_input("Export Filename", value=".pdf")
            ...
            if idx < len(st.session_state.pages_workbench):
                ...
                with col:
                    with st.container(border=True):
                        ...
                        if btn_cols[1].button("➡️", key=f"right_{current_key}",
                                              disabled=(idx == len(st.session_state.pages_workbench) - 1),
                                              help="Move right"):
                            st.session_state.pages_workbench.insert(idx + 1, st.session_state.pages_workbench.pop(idx))
                            st.session_state.final_pdf_bytes = None
                            st.rerun()
                        ...
                        if btn_cols[2].button("🔄", key=f"rot_{current_key}", help="Rotate 90° clockwise"):
                            st.session_state.pages_workbench[idx]["rotation"] = (st.session_state.pages_workbench[idx][
                                                                                     "rotation"] + 90) % 360
                            st.session_state.final_pdf_bytes = None
                            st.rerun()
