...
# --- NEW: Helper function to apply conditional formatting to the SUMMARY dataframe ---
    ...
    try:
        ...
        if not all(col in df_tms_reg.columns for col in tms_cols) or not all(
            ...
            with st.spinner('Processing data...'):
                ...
                if missing_cols:
                    st.error(
                        f"Error: Missing columns in the merged data to perform anomaly checks: {', '.join(missing_cols)}")
                else:
                    ...
                    for index, row in mr.iterrows():
                        ...
                        req_status = str(row['Requisition status']).strip() if pd.notna(
                            row['Requisition status']) else ""
                        ...
                        if state == 'Offer issued' or req_status == 'Offer issued':
                            if pd.notna(row['sys_updated_on_dt']):
                                ...
                                if days_diff > 60:
                                    ...
                                    anomalies.append(
                                        create_record('Offer Issued & Inactive > 60 Days', state, req_status,
                    ...
                    if anomalies:
                        ...
                        # --- NEW: Apply gray styling to Summary DataFrame ---
